...
const MAX_DATE = new Date(now.getTime() + 24 * 60 * 60 * 1000)
  ...
  backup_date: z.string().pipe(
    z.coerce.date()
      ...
      .refine((date) => date.getTime() >= MIN_DATE.getTime(),
        { message: `Backup date must be at least 1 minute from the current time. (Minimum: ${MIN_DATE.toISOString()})` })
      .refine((date) => date.getTime() <= MAX_DATE.getTime(),
        { message: `Backup date cannot be more than 24 hours from the current time. (Maximum: ${MAX_DATE.toISOString()})` })
  ),
})
